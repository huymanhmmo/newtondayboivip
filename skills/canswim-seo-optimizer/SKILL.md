...
1. **Biên tập / Tối ưu hóa**: Đảm bảo bài viết đạt độ dài 3000-5000 từ, có Mục lục, tối thiểu 10 H2 và phân cấp H3, H4. Ghi nhận tác giả **Bùi Văn Cán** kèm ảnh đại diện và bio chuyên nghiệp. Chèn tối thiểu 2 ảnh AI (từ `/images/tuyen-sinh/`) và 3 ảnh thật của bể bơi (từ `/images/` trừ `tuyen-sinh/`). Tạo liên kết nội bộ đến cả 6 trang dịch vụ chính.
2. **Chạy Script kiểm tra**: Chạy công cụ chấm điểm để quét lỗi và lấy điểm số:
   ```bash
   uv run python skills/canswim-seo-optimizer/scripts/seo_check.py check-file src/content/blog/tên-bài-viết.md
   ```
3. **Sửa lỗi & Xuất Schema**: Sửa tất cả các lỗi được báo cáo cho đến khi bài viết đạt điểm tối ưu (>95/100). Sử dụng lệnh sinh schema JSON-LD mẫu để nhúng vào trang:
   ```bash
...
- **Tóm tắt TL;DR / Quick Answer**: Đặt câu trả lời trực tiếp ngắn gọn (40-80 từ) ngay dưới H1 để tăng khả năng được trích dẫn trên Google AI Overviews và Bing Copilot.
